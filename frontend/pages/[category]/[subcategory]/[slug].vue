...
// ── Fetch article (non-blocking so we can show skeleton) ──────────────────────
...
// Hard 404 when done loading and still no article
watchEffect(() => {
...
const breadcrumb = computed(() => [
  ...
  { label: category.value?.name    ?? '',  to: `/${categorySlug}` },
  { label: subcategory.value?.name ?? '',  to: `/${categorySlug}/${subcategorySlug}` },
  { label: article.value?.title    ?? '' },
])

// ── Related articles (same subcategory, parallel fetch) ───────────────────────
  ...
  <div>
    ...
    <template v-else-if="error || !article">
      <div class="flex min-h-[50vh] flex-col items-center justify-center px-4 text-center">
        ...
        <NuxtLink to="/" class="mt-4 rounded-xl bg-brand-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-brand-700">
          {{ t.backToHome }}
        </NuxtLink>
      </div>
    </template>
    ...
    <template v-else>
      ...
      >
        ...
        <!-- Category + title anchored to bottom -->
        <div
      ...
      <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        ...
        <div v-if="tags.length > 0" class="mt-10 border-t border-gray-100 pt-6 dark:border-gray-800">
          <h2 class="mb-3 text-xs font-semibold uppercase tracking-widest text-gray-400 dark:text-gray-500">
            {{ t.tags }}
          </h2>
      ...
      >
        <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
          <h2 class="mb-6 text-xl font-extrabold text-gray-900 dark:text-white">
            {{ t.relatedArticles }}
          </h2>
...
/* ── Prose styles for TipTap HTML output ───────────────────────────────────
   Scoped to .article-content only — does not bleed into other pages.     */
