...
const perm = computed(() => store.currentPermission)
...
async function submit() {
  ...
  store.clearError()
  try {
    await store.update(id, { name: form.name, description: form.description || undefined })
    success.value = true
  } catch {
    // error set in store
  }
...
<template>
  <div class="mx-auto max-w-lg">
    ...
    <div class="rounded-2xl bg-white p-6 shadow-sm">
      ...
      <AppAlert v-if="success" type="success" message="Permission updated successfully." dismissible class="mb-5" @dismiss="success = false" />

      <form v-if="perm" class="space-y-5" @submit.prevent="submit">
        <div>
      ...
      <div v-else class="py-8 text-center text-sm text-gray-400">Permission not found.</div>
    </div>
  </div>
</template>
